...
class CVTransform:
    ...
    def randomFlip(self, img, bboxes):
        # 垂直翻转/y坐标不变，x坐标变化
        if random.random() < opt.aug_threshold:
    ...
    def randomCrop(self, img, bboxes, labels):
        ...
        if random.random() < opt.aug_threshold:
            ...
            if len(bbox_out) == 0:
                ...
                bbox_out[:, [0, 2]] = np.clip(bbox_out[:, [0, 2]] - crop_ymin, a_min=0, a_max=crop_height)
                bbox_out[:, [1, 3]] = np.clip(bbox_out[:, [1, 3]] - crop_xmin, a_min=0, a_max=crop_width)
                new_img = img[crop_ymin:crop_ymax, crop_xmin:crop_xmax, :]
                return new_img, bbox_out, labels_out
        else:
            return img, bboxes, labels
